class Solution:
    ...
    Если новое число оказалось меньше, значит мы закончили расчет предыдущей частичной суммы и готовы добавить новое число.
    Если новое число оказалось равное текущему, значит его надо приплюсовать к текузему.
    ...
    def roman_to_int(self, s: str) -> int:
        ...
        for c in s[1::]:
            if res[-1] > hash_t[c]:
                # если текущее больше нового, значит мы добавляем его в стэк
                res.append(hash_t[c])
            elif res[-1] == hash_t[c]:
